...
final class Util {
     ...
     */
    @Deprecated
    static Calendar getFirstCalendarFormWeekInYear(Context context, int year, int weekInYear) {
        ...
        calendar.setDay(day);
        calendar.setLunar(LunarCalendar.getLunarText(context, calendar.getYear(), calendar.getMonth(), calendar.getDay()));
        return calendar;
    }
     ...
     * @return Calendar
     */
    static Calendar getFirstCalendarFormWeekInYearV2(Context context, int year, int weekInYear) {
        // TODO: 2017/11/23 发生意外bug ，每年的12月和下一年的1月交替的时候需要判断重合的部分
        java.util.Calendar date = java.util.Calendar.getInstance();
        date.set(year, 0, 1);
        int diff = date.get(java.util.Calendar.DAY_OF_WEEK) - 1;//1月第一天为星期几,星期天 == 0，也就是偏移量
